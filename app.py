...
LOAN_GRADE_AUTO_EXPLANATION = (
    "Кредитный рейтинг заявки рассчитывается автоматически на основе введённых "
    "данных. В расчёте учитываются процентная ставка, доля дохода на кредит, "
    "наличие прошлых дефолтов и стаж работы."
)

LOAN_GRADE_RULE_DETAILS = """
**Базовый уровень по процентной ставке:**
- до 8% включительно — A
- 8.1–11% — B
- 11.1–14% — C
- 14.1–17% — D
...
def estimate_loan_grade(loan_int_rate, loan_percent_income, has_previous_default, person_emp_length):
    """Оценивает loan_grade, когда внешний рейтинг заявки не передан."""
    if loan_int_rate <= 8:
        grade_index = 0
...
with tab3:
    ...
    В этой системе рейтинг рассчитывается автоматически по введённым параметрам.
    Основной ориентир — процентная ставка по кредиту. Дополнительно учитываются
    долговая нагрузка, наличие прошлых дефолтов и стаж работы. Такой подход
    снижает субъективность: пользователь заполняет фактические данные, а рейтинг
    формируется на их основе.
    """)
    with st.expander("Как формируется рейтинг"):
        st.markdown(LOAN_GRADE_RULE_DETAILS)
